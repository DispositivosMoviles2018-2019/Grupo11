package com.example.ludl.appmovement;
...
public class UserMotionService extends IntentService {
    ...
    public interface OnActivityChangedListener {
        void onUserActivityChanged(int bestChoice, int bestConfidence, ActivityRecognitionResult newActivity);
    }
    ...
    protected void onHandleIntent(Intent intent) {
        if (ActivityRecognitionResult.hasResult(intent)) {
            ...
            if (activity.getType() == DetectedActivity.UNKNOWN && activity.getConfidence() < 60 && result.getProbableActivities().size() > 1) {

                activity = result.getProbableActivities().get(1);
            }


            if (mLastKnownActivity == null || mLastKnownActivity.getType() != activity.getType() || mLastKnownActivity.getConfidence() != activity.getConfidence()) {

                Message msg = Message.obtain(null, 0,                         //what
